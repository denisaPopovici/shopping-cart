...
public class JavaMailUtil {
	...
	public static void sendMail(String recipientMailId) throws MessagingException {
		Session session = prepareMail();

		Message message = prepareMessage(session, ResourceBundle.getBundle("application").getString("mailer.email"), recipientMailId);

		Transport.send(message);

		Logger.getAnonymousLogger().log(Level.INFO, "Message Sent Successfully!");
	}

	private static Session prepareMail() {
		Logger.getAnonymousLogger().log(Level.INFO, "Preparing to send Mail");
	...
	protected static void sendMail(String recipient, String subject, String htmlTextMessage) throws MessagingException {
		...
		Message message = prepareMessage(session, ResourceBundle.getBundle("application").getString("mailer.email"), recipient, subject, htmlTextMessage);
